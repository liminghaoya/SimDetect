...
class AC(BackdoorDefense):
    ...
    def detect(self, inspect_correct_predition_only=True, noisy_test=False):
        ...
        print("Suspicious Class:", suspicious_class.item())
        ...
        if inspect_correct_predition_only:
            ...
            # 确保 y_true, y_pred, y_score 和 mask 都在同一个设备上
            y_true = y_true.to(device)
            y_pred = y_pred.to(device)  # 添加 y_pred 移动到同一设备
            y_score = y_score.to(device)  # 确保 y_score 也在同一设备
            mask = mask.to(device)
