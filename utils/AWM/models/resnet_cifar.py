...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1, norm_layer=None):
        ...
        self.conv2 = MaskedConv2d(planes, planes, kernel_size=3,
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1, norm_layer=None):
        ...
        self.bn2 = norm_layer(planes)
        self.conv3 = MaskedConv2d(planes, self.expansion *
...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=43, norm_layer=None):
        ...
        self.conv1 = MaskedConv2d(3, 64, kernel_size=3,
                               stride=1, padding=1, bias=False)
...
def resnet18(num_classes=43, norm_layer=nn.BatchNorm2d):
    return ResNet(BasicBlock, [2, 2, 2, 2], num_classes, norm_layer)


def resnet34(num_classes=43, norm_layer=nn.BatchNorm2d):
    return ResNet(BasicBlock, [3, 4, 6, 3], num_classes, norm_layer)


def resnet50(num_classes=43, norm_layer=nn.BatchNorm2d):
    return ResNet(Bottleneck, [3, 4, 6, 3], num_classes, norm_layer)


def resnet101(num_classes=43, norm_layer=nn.BatchNorm2d):
    return ResNet(Bottleneck, [3, 4, 23, 3], num_classes, norm_layer)


def resnet152(num_classes=43, norm_layer=nn.BatchNorm2d):
