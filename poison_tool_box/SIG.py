...
class poison_generator():
    ...
    def generate_poisoned_training_set(self):
        ...
        print('all_target_indices',len(all_target_indices))

        # 选择是否要随机选取部分样本1是，0否
        if_rand_daset = 0
        if if_rand_daset == 1:
            # 选取其中1000个样本
            clean_split_indices = all_target_indices[:1000]
            self.dataset = torch.utils.data.Subset(self.dataset, clean_split_indices)
            self.num_img = len(self.dataset)
            print('Selected 1000 sample indices:', clean_split_indices)
            print('adad', self.num_img)

        num_poison = int(self.num_img * self.poison_rate)
        poison_indices = all_target_indices[:num_poison]
        poison_indices.sort()  # increasing order

        print('poison_indices :', len(poison_indices))
